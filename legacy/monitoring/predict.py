...
def filter_false_positives(df_predictions):
    ...
    print(f"ðŸŽ¯ Filtered {len(df_predictions) - len(df_filtered)} false positives")
    
    return df_filtered

def predict_with_ensemble():
    print("ðŸ” Starting enhanced ensemble phishing domain prediction...")
    ...
    try:
        ...
        print(f"âŒ Model file not found: {e}")
        return None
    
    # Load CSE reference data
    cse_domain_to_name, validation_patterns = load_cse_reference()
    print("âœ… Loaded CSE reference data")
    ...
    # ðŸŽ¯ CRITICAL FIX: Filter domains to only those targeting CSEs
    print("ðŸŽ¯ Filtering domains to CSE-targeting only...")
    ...
    print(f"ðŸ“Š Reduced from {initial_count:,} to {len(df_targeted):,} targeted domains")
    
    if len(df_targeted) == 0:
        print("âŒ No CSE-targeting domains found")
        return None
    
    # Lexical prediction
    print("ðŸ” Extracting lexical features...")
    ...
    # ðŸŽ¯ IMPROVED CSE target identification
    print("ðŸŽ¯ Identifying targeted CSEs...")
    cse_targets = []
    cse_domains = []
    cse_confidences = []
    ...
    # ðŸŽ¯ REMOVE CV/OCR - it's causing more problems than solutions
    ...
    # from herald.features.content_classifier import ContentClassifier
    
    print("\nðŸŽ¯ Applying two-stage classification...")
    ...
    print(f"\nðŸ“Š TWO-STAGE CLASSIFICATION SUMMARY:")
    ...
    print(f"ðŸŽ¯ CSE-targeting phishing saved ({len(cse_phishing)} domains)")
    ...
    # Show top examples
    print(f"\nðŸ” TOP CSE-TARGETING DOMAINS:")
    sample_cse = cse_phishing.head(10)
    for _, row in sample_cse.iterrows():
        print(f"   - {row['domain']} â†’ {row['target_cse']} (Confidence: {row['confidence']:.4f})")
