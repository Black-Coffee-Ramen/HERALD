"""
scripts/retrain_v4.py

Retrains the XGBoost + Random Forest ensemble on the full v4 feature set.
...
Architecture: Same as v3
  ...
  - Binary label: Phishing = 1, Suspected/Legitimate = 0

Input:  data/processed/full_features_v4.csv
Output: models/ensemble_v4.joblib
        outputs/retrain_v4_results.csv
"""
...
def main():
    print("=" * 60)
    print("HERALD Ensemble Retraining  —  v4")
    print("=" * 60)
    ...
    print("\nTraining final v4 ensemble on full training set...")
    ...
    # ─── Validation Set ───────────────────────────────────────────────────────
    val_probs = (xgb_v4.predict_proba(X_val)[:, 1] + rf_v4.predict_proba(X_val)[:, 1]) / 2
    val_preds = (val_probs >= THRESHOLD).astype(int)
    print("\n--- VALIDATION SET ---")
    print(classification_report(y_val, val_preds, target_names=['Non-Phishing', 'Phishing']))

    # ─── Test Set ─────────────────────────────────────────────────────────────
    test_probs = (xgb_v4.predict_proba(X_test)[:, 1] + rf_v4.predict_proba(X_test)[:, 1]) / 2
    ...
    print(f"\nPrecision: {p:.3f}  (target >= 0.90)")
    print(f"Recall:    {r:.3f}  (target >= 0.92)")
    print(f"F1:        {f:.3f}")

    if p >= 0.90 and r >= 0.92:
        print("\nTargets MET.")
    else:
        missed = []
        if p < 0.90: missed.append(f"precision {p:.3f} < 0.90")
    ...
    results_path = 'outputs/retrain_v4_results.csv'
    results = {
        'dataset_size': len(df),
