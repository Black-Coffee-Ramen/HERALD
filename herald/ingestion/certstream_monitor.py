...
def print_callback(message, context):
    ...
    if message['message_type'] == "certificate_update":
        ...
        for domain in all_domains:
            ...
            if is_susp:
                logging.warning(f"ðŸš¨ SUSPICIOUS CERT DETECTED: {clean_domain} -> Target: {target_cse}")
            ...
            if response.status_code == 200:
                ...
                for entry in data[:50]:  # Limit to recent 50 to avoid overloading queue
                    ...
                    for d in domain.split('\n'):
                        ...
                        is_susp, target_cse = is_suspicious(clean_domain)
                        if is_susp:
                            logging.warning(f"ðŸš¨ SUSPICIOUS CERT (Fallback): {clean_domain} -> Target: {target_cse}")
                            if redis_client:
